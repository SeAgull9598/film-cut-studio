...
LP.help = (function () {
  ...
  function aiFill(shot) {
    ...
    guide.appendChild(el('div', {
      class: 'm-d',
      text: '到底部「素材包 / 报告」页，把维度＋文字＋转写导出成结构化素材包，交给 WorkBuddy 出完整报告——质量最高，而且素材依然不出机。'
    }));
  ...
  function welcome(force) {
    ...
    [
      ...
      ['④ 导出素材包，让 AI 出报告', '到底部「素材包 / 报告」页，一键生成结构化文本包，复制给 WorkBuddy 出完整分析。默认不调任何云端接口——这是隐私红线，也是这个工具的前提。']
    ].forEach(function (s) {
